...
parser.add_argument('--use_icp_pp', default= False, type=bool)
parser.add_argument('--fitting_loss', default= "chamfer", type=str)
...
USE_ICP_PP = FLAGS.use_icp_pp
FITTING_LOSS = FLAGS.fitting_loss
	...
	if (TO_TRAIN):
		...
		for epoch in range(n_epochs):
			...
			for i, batch in enumerate(loader):
				...
				### Bool to update fitting network
				to_update_fitting = (not FIXED_DEFORMATION) and ((INIT_DEFORMATION and i%2==0) or ((not INIT_DEFORMATION) and (epoch<20 or i%2==0)))

				if (USE_ICP_PP) and to_update_fitting:
					## Output shape with ICP
					'''
					Direct fitting via optimization
					'''
					param_init = get_param_init(params, def_param, ALPHA)
					# param_init = torch.tensor(param_init)
					param_init = param_init.clone().detach().to(device)
					param_init = param_init.view(param_init.shape[0], param_init.shape[2], param_init.shape[1])
					param_init.requires_grad_(True)
					
					# icp_learning_rate = 0.05
					icp_learning_rate = 0.5
					...
					for icp_epoch in range(icp_n_iters):
						...
						if (USE_CONNECTIVITY):
							#Project param using constraint
							# param_cons = torch.bmm(conn_mat, def_param)
							# output_pc = icp_forward(mat, param_cons)
							output_pc = icp_forward(mat, param_init, connectivity_mat=conn_mat)

						else:
							# print(mat.shape)
							# print(def_param.shape)
							output_pc = icp_forward(mat, param_init)


						# zero the gradients after updating
						optimizer.zero_grad()

						# l, _ = chamfer_distance(output_pc, x_repeat)

						# # backprop
						# l.backward(retain_graph=True)

						l, _ = chamfer_distance(output_pc, x_repeat, batch_reduction=None)

						if USE_SYMMETRY:
							reflected_pc = get_symmetric(output_pc)
							symmetric_loss, _ = chamfer_distance(output_pc, reflected_pc, batch_reduction=None)
							l += symmetric_loss
							# print("using symmetry loss")
						...
						if icp_epoch % 10 == 0:
						    print('epoch ', icp_epoch+1, ' loss = ', loss.data.cpu().numpy())	

						# curr_loss = l.data.cpu().numpy()
						...
						if (prev_cd_losses is not None):
							diff_cd = curr_cd_losses - prev_cd_losses
							diff_cd  = diff_cd.to("cpu").detach().numpy()

							if (np.max(abs(diff_cd)) < 1e-5):
								break

						prev_cd_losses = curr_cd_losses	


					param_init = param_init.view(param_init.shape[0], param_init.shape[2], param_init.shape[1])
					param_init = uninit_param(param_init, def_param, ALPHA)
					################

				else:
					## Output shape with the network
					if (USE_CONNECTIVITY):
				...
				#Update deformation network				
				if to_update_fitting:

					if (FITTING_LOSS == "l2") and (USE_ICP_PP):
						print("Using L2")
						## Direct L2 supervision from ICP fitting
						...
						if (SELECTION == "retrieval_candidates" or SELECTION == "deformation_candidates"):
							if (to_select_from_dict):
								...
								params = params.view(K, -1, params.shape[1])
								param_init = param_init.view(K, -1, param_init.shape[1])

								# print(params.shape)
								# print(param_init.shape)
								# print(weights.shape)

								loss = torch.sum(torch.square(params - param_init), dim=-1)
								...
								fitting_loss = torch.sum(torch.square(params - param_init), dim=1)
								fitting_loss = torch.mean(fitting_loss)
						else:				
							fitting_loss = torch.sum(torch.square(params - param_init), dim=1)
							fitting_loss = torch.mean(fitting_loss)
						############

					elif (FITTING_LOSS == "SS") and (USE_ICP_PP):
						## Chamfer with ICP fit model
						print("Using SS")

						## Output shape with the network
						if (USE_CONNECTIVITY):
							network_pc = get_shape(mat, params, def_param, ALPHA, connectivity_mat=conn_mat)
						else:
							network_pc = get_shape(mat, params, def_param, ALPHA)

						cd_loss, _ = chamfer_distance(output_pc, network_pc, batch_reduction=None)
						loss = cd_loss
					...
					log = "{} | Batch [{:04d}/{:04d}] | fitting loss: {:.6f} |"
		...
		print("ERROR. Use evaluate_images.py")
