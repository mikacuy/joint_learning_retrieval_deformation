...
parser.add_argument("--logdir", default="log_chair_keypoint_comparisons_C2_srcenc_symm/", help="path to the log directory", type=str)
parser.add_argument('--dump_dir', default= "dump_chair_keypoint_comparisons_C2_srcenc_symm/", type=str)
...
LOG_FOUT = open(os.path.join(DUMP_DIR, 'log_ranked_evaluation.txt'), 'w')
...
if __name__ == "__main__":
	...
	# sources, train_targets, test_targets = get_all_selected_models_pickle(FILENAME)
	...
	# if (DATA_SPLIT=="train"):
	# 	dataset = StructureNetDataset_Multi_MemEff(data_fol, sources, train_targets)	
	# else:
	# 	dataset = StructureNetDataset_Multi_MemEff(data_fol, sources, test_targets)	
	if COMPLEMENTME:
	...
	if not (os.path.exists(filename)):	
		...
		# fname = os.path.join(LOG_DIR, "checkpoint_0199.pth")
		...
		all_ranked_retrieved = np.zeros(len(SOURCE_MODEL_INFO))
