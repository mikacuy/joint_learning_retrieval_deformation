...
parser.add_argument('--selection', default= "retrieval_candidates", type=str)
...
parser.add_argument('--init_joint', default= False, type=bool)
...
parser.add_argument('--model_init', default= "log_chair500_conn_images/", type=str)
...
#Autodecoder Initialization
parser.add_argument('--ad_init', default= "random", type=str)
...
if __name__ == "__main__":
	...
	if (AD_INIT == "smart"):
		...
		### Load a model from the deformation network and use encoder vectors as initialization
		target_encoder_init = TargetEncoder(
		    TARGET_LATENT_DIM,
		    args["input_channels"],
		)
		target_encoder_init.to(device, dtype=torch.float)
		fname = os.path.join("log_chair500_conn/", "model.pth")
		target_encoder_init.load_state_dict(torch.load(fname)["target_encoder"])
		target_encoder_init.to(device)
		target_encoder_init.eval()	
	...
	for source_model in sources:
		...
		if (AD_INIT == "smart"):
			with torch.no_grad():
				src_points = np.expand_dims(points, axis=0)
				src_points = torch.from_numpy(src_points).float().to(device)
				curr_src_init_code = target_encoder_init(src_points)
				RETRIEVAL_SOURCE_LATENT_CODES.append(curr_src_init_code[0])

	print(len(SOURCE_MODEL_INFO))
	print(len(SOURCE_PART_LATENT_CODES))

	## Create source latent
	SOURCE_LATENT_CODES = torch.autograd.Variable(torch.randn((len(SOURCE_MODEL_INFO),SOURCE_LATENT_DIM), dtype=torch.float, device=device), requires_grad=True)

	if not SHARE_SRC_LATENT:
		if (AD_INIT == "random"):
			RETRIEVAL_SOURCE_LATENT_CODES = torch.autograd.Variable(torch.randn((len(SOURCE_MODEL_INFO),SOURCE_LATENT_DIM), dtype=torch.float, device=device), requires_grad=True)

		elif (AD_INIT == "smart"):
			RETRIEVAL_SOURCE_LATENT_CODES = torch.stack(RETRIEVAL_SOURCE_LATENT_CODES)
			RETRIEVAL_SOURCE_LATENT_CODES = RETRIEVAL_SOURCE_LATENT_CODES.clone().detach().to(device)
			RETRIEVAL_SOURCE_LATENT_CODES.requires_grad_(True)


	if (DIST_FUNC == "mahalanobis"):
		if (AD_INIT == "random"):
			SOURCE_VARIANCES = torch.autograd.Variable(torch.randn((len(SOURCE_MODEL_INFO),SOURCE_LATENT_DIM), dtype=torch.float, device=device), requires_grad=True)

		#Smarter initialization
		elif (AD_INIT == "smart"):
			SOURCE_VARIANCES = torch.autograd.Variable(torch.ones((len(SOURCE_MODEL_INFO),SOURCE_LATENT_DIM), dtype=torch.float, device=device), requires_grad=True)


	if (LOSS_FUNC == "regression"):
		if (AD_INIT == "random"):
			SOURCE_SIGMAS = torch.autograd.Variable(torch.randn((len(SOURCE_MODEL_INFO),1), dtype=torch.float, device=device), requires_grad=True)

		#Smarter initialization
		elif (AD_INIT == "smart"):
			SOURCE_SIGMAS = torch.autograd.Variable(torch.ones((len(SOURCE_MODEL_INFO),1), dtype=torch.float, device=device), requires_grad=True)
	...
	def construct_candidates_dict(loader, encoder):
		...
		for i, batch in enumerate(loader):
			...
			target_images, target_shapes, target_ids, _, semantics = batch
			x = [x.to(device, dtype=torch.float) for x in target_images]
			x = torch.stack(x)
			...
			if USE_SRC_ENCODER_RETRIEVAL:
				with torch.no_grad():
					...
					for j in range(num_sets):
						if (j==num_sets-1):
							curr_src_latent_codes = get_source_latent_codes_encoder_images(source_labels[j*interval:], SOURCE_MODEL_INFO, encoder, device=device, obj_cat=OBJ_CAT)
						else:
							curr_src_latent_codes = get_source_latent_codes_encoder_images(source_labels[j*interval:(j+1)*interval], SOURCE_MODEL_INFO, encoder, device=device, obj_cat=OBJ_CAT)
	...
	def construct_deformation_candidates_dict(loader, encoder):
		...
		for i, batch in enumerate(loader):
			...
			target_latent_codes = encoder(im)
			...
			for j in range(num_sub_epochs):
				...
				# #Debug
				# print(curr_source_labels.shape)
				# print(curr_target_latent_codes.shape)
				# print(curr_x_repeated.shape)	
				# exit()			
		...
		for epoch in range(n_epochs):
			...
			for i, batch in enumerate(loader):
				...
				if USE_SRC_ENCODER:
					...
					src_latent_codes = get_source_latent_codes_encoder_images(source_labels, SOURCE_MODEL_INFO, target_encoder, device=device, obj_cat=OBJ_CAT)
				...
				# elif FIXED_DEFORMATION or (INIT_DEFORMATION and i%2==1) or (epoch >=20 and i%2==1):
					...
					### Embedding loss
					if not SHARED_ENCODER:
						retrieval_latent_codes_single = retrieval_encoder(im)
					...
					if USE_SRC_ENCODER_RETRIEVAL:
						##Use the encoder to get the source latent code
						src_latent_codes = get_source_latent_codes_encoder_images(source_labels, SOURCE_MODEL_INFO, retrieval_encoder, device=device, obj_cat=OBJ_CAT)
